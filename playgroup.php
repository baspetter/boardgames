...
<div style="display:flex;flex-wrap:wrap;align-items:center;gap:1rem;background:var(--surface);border:1px solid var(--border);border-radius:0.5rem;padding:1rem;margin:1rem 0 2rem;">
  <div>
    <p class="section-label" style="margin-bottom:0.2rem;">Uitnodigingscode</p>
    <p style="font-family:monospace;font-size:1.1rem;letter-spacing:0.1em;color:var(--accent);margin:0;"><?= h($group['invite_code']) ?></p>
  </div>
  <?php if ($canManage): ?>
    <form method="post" onsubmit="return confirm('Weet je zeker dat je een nieuwe code wilt genereren? De oude werkt dan niet meer.');">
      <input type="hidden" name="action" value="regenerate_code">
      <button type="submit" class="btn btn-secondary" style="font-size:0.75rem;">Nieuwe code genereren</button>
    </form>
  <?php endif; ?>
...
<h2>Gezamenlijke collectie</h2>
<?php render_game_grid($collection, 'Niemand in deze groep heeft nog spellen toegevoegd.'); ?>
